...
import { loginUser,registerUser } from "../States/AUTH/Action";
...
export default function AuthPage() {
  ...
  console.log("Auth state:", { loading, error, user, isAuthenticated });
  ...
  useEffect(() => {
    // whenever auth state updates we redirect depending on role or authentication status
    if (isAuthenticated && user) {
      ...
      if (userRole === "ADMIN") {
        ...
        toast.success('Welcome back! Redirecting to home...', { autoClose: 2000 });
    ...
    if (!isLogin) {
      ...
      try {
        ...
        if (!res?.error) {
          ...
          // notify any listener (header etc) that storage changed
          window.dispatchEvent(new Event('authChanged'));
